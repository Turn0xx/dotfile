...
import { ActionMap, ObservableAction, Subject } from "./building-blocks/observability/subject";
...
export class TextAreaSetting implements Subject{
	...
	notify(payload: ActionMap['updated']): void {
		
		this.observers.map((observer) => {
			switch (payload.action) {
				case "add":
					observer.notifyAdd(payload.colorName);
					break;
				case "remove":
					observer.notifyRemove(payload.colorName);
					break;
				case "updated":
