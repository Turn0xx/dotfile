...
export class ValidateBasicInformationUseCase {
  ...
  async handle(command: ValidateBasicInformationCommand) {
    ...
    const basicInfos: EventBasicInformationsJson = {
      id: command.id,
    ...
    const organizer = await this.organizerRepository.findById(
      command.organizerId,
    );
    ...
    organizer.initializeEvent(EventBasicInformations.create(
      basicInfos.id,
      organizer.getId(),
      basicInfos.basicInfo,
      basicInfos.place,
      basicInfos.date,
      basicInfos.createdAt,
    ));

    await this.basicInformationRepository.save(basicInfo);
  }
}
