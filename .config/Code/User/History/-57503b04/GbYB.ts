...
class ValueObject<Primitive> {
  ...
  static fromValue<Primitive>(value: unknown , validator ?: (value: unknown) => boolean): ValueObject<Primitive> {
    if (validator) {
      if (!validator(value)) {
        throw new Error('Invalid value');
      }
    }
    return new ValueObject(value as Primitive);
  }
...
class Super extends ValueObject<string> {
