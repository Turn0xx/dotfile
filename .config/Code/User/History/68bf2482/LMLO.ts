...
export class Colorizer {
	...
	public static uncolor() {
		...
		// Remplacer la balise <font> trouvée par son contenu
		const newLine =
			currentLine.substring(0, closestFontTag.index) +
			closestFontTag[1] +
			currentLine.substring(fontRegex.lastIndex);
		editor.setLine(cursor.line, newLine);

		// Garder le curseur à la même position relative, ajustée pour les changements de longueur de texte
		let newCursorPos =
			cursor.ch - (closestFontTag[0].length - closestFontTag[1].length);
		if (cursor.ch > closestFontTag.index + closestFontTag[0].length) {
			// Si le curseur est après la balise <font> modifiée, ajustez sa position
			newCursorPos = Math.max(
				cursor.ch -
					(closestFontTag[0].length - closestFontTag[1].length),
				closestFontTag.index
			);
		}
