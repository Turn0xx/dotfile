...
export class Colorizer {
	...
	public static uncolor() {
		...
		const afterTag1 = currentLine.substring(endOfTag);
		const afterTag2 = currentLine.substring(contentOfTag.length + 1 , endOfTag);
		const contentOfTag2 = contentOfTag
		...
		const newLine =
			...
			contentOfTag +
			currentLine.substring(contentOfTag.length + 1 , endOfTag);
