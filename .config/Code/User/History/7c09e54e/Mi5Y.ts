...
export class ColorSettingsTab extends PluginSettingTab {
  ...
  public displayColorSettings(): void {
    ...
    this.colorSettings
      ...
      .setClass("text-colors-input")
      .addTextArea(async (textArea) => {
        await this.textAreaSetting.configure(textArea, this.colorSettings);
      });
  ...
  public displayPreviewColorSettings(): void {
    let { containerEl } = this;
    ...
    this.textAreaSetting.attach(ColorPreview.fromColors(this.colorManager.colors));
    this.textAreaSetting.render(this.previewColorSettings);
  }
}
