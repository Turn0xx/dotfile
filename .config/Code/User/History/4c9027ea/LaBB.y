...
int main(int argc, char** argv) {
    if(argc != 2) {
        fprintf(stderr, "Usage: ./parseur <input file>\n");
        exit(1);
    }

    ast_array = malloc(sizeof(T*) * 1000);
    quadStack = create_stack();
    ...
    dataSec = initDataSection();
    fs_open("output.asm");
    initializeScopeTree();
    yyparse();
    fclose(yyin);
    fs_writef("");
    fs_writef("# COPY PRINT PARAMETERS HERE\n");
    fs_writef("li $v0, 10\nsyscall\n");
    fs_close();
    mips_write_datasection();
