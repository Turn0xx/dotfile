...
describe('Support Feature : Forgot Password', async () => {
  ...
  describe('Failing Scenario: Client forgot his password', () => {
    it('When Client forgot his password and uses email', async () => {
      ...
      await fixture.thenErrorShouldBe(new Error('Client not found'));
    });
  });
});
...
const createFixture = () => {
  ...
  return {
    ...
    thenErrorShouldBe: (error: Error) => {
      expect(throwError).toBeInstanceOf(typeof error);
    },
    ...
    andDigitCodeShouldBeInserted: async (clientId: number) => {
      expect(tokensRepository.getDigitCodeByClientId(clientId)).toBeDefined();
    }
  };
};
