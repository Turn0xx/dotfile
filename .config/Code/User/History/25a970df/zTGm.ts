...
const createFixture = () => {
  ...
  let forgotPasswordUseCase = new ForgotPasswordUseCase(
  ...
  return {
    ...
    givenSomeOneIsRegistered: async (email: string , phone: string) => {
      const client = clBuilder.withEmail(email).withPhoneNumber(phone).toIndividualClient('John', 'Doe').build();
      const clientSchema =  IndividualClient.fromJsonToDataModel(client);

      await clientRepository.insert(clientSchema);
    },

    whenClientForgotHisPassword: async (id: string, identification: string , type: string) => {
      try {
