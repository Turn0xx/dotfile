...
export class ClientTypeOrmRepository implements ClientRepository {
  constructor(private readonly clientRepository: Repository<ClientSchema>) {}
  ...
  async insert(client: ClientSchema): Promise<ClientSchema> {
    try {
      ...
      // const retClient = awaie this.clientRepository.insert(client);
  ...
  async findByEmail(email: string): Promise<Client> {
    try {
      ...
      console.log('error in find by email')
      throw new Error(error);
    }
  }
  ...
  async findByPhone(phone: string): Promise<Client> {
    try {
      const client = await this.clientRepository.findOne({ where: { phoneNumber: phone } });
