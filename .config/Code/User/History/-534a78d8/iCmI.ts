...
import { basicInformationBuilder } from 'src/pocket-ticket/client/test/builders/basic-information.command.builder';
import { EventBasicInformations } from 'src/pocket-ticket/client/domain/basic-information';
...
describe('OrganizerRepo', () => {
  ...
  it('should save an organizer with events', async () => {
    ...
    const organizerToSave = Organizer.fromJson(organizerStub.withEvents(
      [{
        id: 1,
        basicInformations: EventBasicInformationsBasicInformation.fromJson(basicInformation),
        createdAt: new Date(),
        isPublished: false,
      }]
    ).build());
