...
export class IndividualClient extends Client {
  ...
  static fromJsonToDataModel(json: IndividualClientJson): IndividualDataModel {
    ...
    this.validateValueObjects(json);
  ...
  private static validateValueObjects(json: IndividualClientJson) {

    if (!PhoneNumber.validator(json.phoneNumber)) {
      throw new DomainVoValidationError('Client', 'PhoneNumber', 'Invalid phone number');
    }
    ...
    if (!Password.validator(json.password)) {
      throw new DomainVoValidationError('Client', 'Password', 'Invalid password');
    }
