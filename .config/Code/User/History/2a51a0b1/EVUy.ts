...
} from 'src/pocket-ticket/client/authentification/application/usecases/validate-basic-information.usecase';
import {
  Organizer,
  OrganizerJson,
} from 'src/pocket-ticket/client/authentification/domain/organizer';
import { InMemoryBasicInformationRepository } from 'src/pocket-ticket/client/authentification/test/basic-information.in-memory';
import {
  BasicInformationBuilder,
  basicInformationBuilder,
} from 'src/pocket-ticket/client/authentification/test/builders/basic-information.command.builder';
import { organizerBuilder } from 'src/pocket-ticket/client/authentification/test/organizer.builder';
import {
  InMemoryOrganizerRepository,
  StubDateProvider,
} from 'src/pocket-ticket/client/authentification/test/organizer.in-memory';
...
const createFixture = () => {
  ...
  return {
    ...
    thenInformationsShouldBeSaved: async (builder: BasicInformationBuilder) => {
      ...
      const basicInformation = await organizerRepository.findById(1);

      const basicInfo = builder.build();

      expect(basicInformation.getEvents().find(
        (event) => event.getId() === basicInfo.id,
      ).getBasicInformations()).toEqual({
        ...basicInfo,
