...
export class InMemoryClientRepository implements ClientRepository {
  ...
  async insert(client: ClientSchema): Promise<ClientSchema> {
    const difiriencedClient =
      client.accountType === AccountTypeEnum.INDIVIDUAL
  ...
  save(clientSchema: ClientSchema): Promise<ClientSchema> {
    const client =
      clientSchema.accountType === AccountTypeEnum.INDIVIDUAL
        ? IndividualClient.fromDataModel(clientSchema)
        : CompanyClient.fromDataModel(clientSchema);
  ...
  async findByPhone(phone: string): Promise<Client | undefined> {
    return await Promise.resolve(
      Array.from(this.clients.values()).find((c) => c.PhoneNumber === phone),
    );
  }

  async verifyClient(id: string): Promise<void> {
