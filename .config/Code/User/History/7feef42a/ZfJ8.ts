...
export class TextAreaSetting {
	...
	public async configure(
		...
		TextComponent.onChange(async () => {
			...
			this.removeDoublon();
			this.saveColors(this.lastValidValue);
		});
	}
	...
	private removeDoublon() {
		this.lastValidValue = [...new Set(this.lastValidValue.split(";"))].join(
			";"
		);
		this.textAreaRef.setValue(this.lastValidValue);
	}
}
