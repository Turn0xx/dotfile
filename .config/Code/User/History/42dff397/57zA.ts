...
describe('Password Reset Process', async () => {
  ...
  describe('Passing Scenario: Client forgot his password and want to reset', () => {
    test('Client should receive a code digit and validate it', async () => {
      await fixture.whenUserForgotHisPassword({
        identificator: testClient.build().email,
        type: 'email',
      });

      const tokenSent = await fixture.thenHeShouldReceiveACodeDigit();

      const res = await fixture.whenUserEntersTheCode(tokenSent.token);

      fixture.thenItShouldPass(res);
    });    
  });

  describe('Failing Scenario: Client forgot his password and want to reset', () => {
    test('Client try an invalid digit code', async () => {
      await fixture.whenUserForgotHisPassword({
        identificator: testClient.build().email,
        type: 'email',
      });

      await fixture.thenHeShouldReceiveACodeDigit();

      const res = await fixture.whenUserEntersTheCode('123456');

      fixture.thenItShouldFail(res);
    });
  });
});
...
const createFixture = (props: {
  ...
  // let tokenSent: string = undefined
  ...
  return {
    ...
    whenUserForgotHisPassword: async (payload: {
      ...
      const req = await request(app.getHttpServer())
        .post(
          `/identity/forgot/password?identificator=${payload.identificator}&type=${payload.type}`,
        )
        .send();
    ...
    thenHeShouldReceiveACodeDigit: async () => {
      const token = await tokensRepository.findOne({
        where: { client: { id: testClientSchema.id } },
      });

      expect(token).toBeDefined();

      // tokenSent = token.token;
    ...
    whenUserEntersTheCode: async (code: string) => {
      try {
        const req = await request(app.getHttpServer()).get(
          `/identity/forgot/password/validate?token=${code}`,
        );
        return req;
      } catch (error) {
        console.log(error);
    ...
    thenItShouldPass: (error) => {
      expect(error.status).toBe(HttpStatus.OK);
    },
    thenItShouldFail: (error) => {
      expect(error.status).toBe(HttpStatus.FORBIDDEN);
    }

    // getTokenSent: () => tokenSent,
  };
};
