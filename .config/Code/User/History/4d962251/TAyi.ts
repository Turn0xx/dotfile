...
import { EmailSender } from '../../../../src/pocket-ticket/notification-services/emails/application/email-sender';
import { emailValidationServiceProvider } from '../../../../src/pocket-ticket/notification-services/emails/framework/email-validation-service.provider';
import { EmailsModule } from '../../../../src/pocket-ticket/notification-services/emails/emails.module';
import { NotificationsModule } from '../../../../src/pocket-ticket/notification-services/notification.module';
import { IdentityModule } from '../../../../src/pocket-ticket/client/identity/identity.module';
...
describe('Registrations Utilities', () => {
  ...
  beforeEach(async () => {
    const moduleFixture = await Test.createTestingModule({
      imports: [AppModule , IdentityModule , EmailsModule , NotificationsModule],
    })
      .overrideProvider(emailValidationServiceProvider)
      .useFactory({
        factory: (
          sendingStrategy: EmailSender,
          tokenRepository: TokensRepository,
        ) => {
          console.log('sendingStrategy', sendingStrategy);
          return new EmailValidationService(tokenRepository, sendingStrategy);
        },
        inject: [FakeEmailSender, PROVIDERS.TOKENS_REPOSITORY],
      })
      .overrideModule(EmailsModule)
      .useModule(
        {
          module: EmailsModule,
          providers: [
            {
              provide: PROVIDERS.EMAIL_SENDER,
