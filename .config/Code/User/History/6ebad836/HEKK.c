...
void printEntireSymbolTable(){
    TableSymScope *root = ST.root;

    printf("Printing entire symbol table\n");
    printf("Root scope\n");
    for(int i = 0; i < MAX_SYMBOLS; i++){
        if(root->symbolTable[i].type != -1){
            printf("Name: %s, Type: %d, Address: %d\n", root->symbolTable[i].name, root->symbolTable[i].type, root->symbolTable[i].address);
        }
    }

    for(int i = 0; i < root->numChildren; i++){
        printf("Child scope %d\n", i);
        for(int j = 0; j < MAX_SYMBOLS; j++){
            if(root->childrenScope[i]->symbolTable[j].type != -1){
                printf("Name: %s, Type: %d, Address: %d\n", root->childrenScope[i]->symbolTable[j].name, root->childrenScope[i]->symbolTable[j].type, root->childrenScope[i]->symbolTable[j].address);
            }
        }
    }
