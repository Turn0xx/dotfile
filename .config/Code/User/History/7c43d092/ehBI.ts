...
describe('TokensTypeormRepository', () => {
  ...
  beforeEach(async () => {
    ...
    await typeormClientRepository.save(IndividualClient.fromJsonToDataModel(individualClientBuilder().build()));
  });
  ...
  describe('insertToken', () => {
    it('should insert a token', async () => {
      ...
      typeormRepository.manager.createQueryBuilder().from(TokenSchema, 'token').;
      expect(token).toBeDefined();
      expect(token.token).toBe('token');
      expect(token.client.id).toBe(1);
    ...
    it('should throw an error when the token already exists', async () => {
      await tokensRepository.insertToken('token', 1);
      try {
        await tokensRepository.insertToken('token', 1);
      } catch (error) {
