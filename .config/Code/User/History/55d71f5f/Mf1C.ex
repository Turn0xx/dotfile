defmodule TimeManagerWeb.TimeTracking.Infrastructure.WorkingTimeController do
  ...
  def show(conn, %{"userID" => userID, "start_time" => start, "end_time" => end_time}) do
    with {:ok,[%WorkingTime{}] = working_times} <- ManageWorkingTimeService.get_work_time_by_user_id_and_time_range(%{"userID" => userID, "start_time" => start, "end_time" => end_time}) do
      conn
      |> put_status(:ok)
      |> put_view(WorkingTimePresenter)
      |> render(:present_working_time, working_times: working_times)
    end
  end
