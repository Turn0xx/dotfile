  ...
  def index(conn, params) do
    case params do
      %{"email" => email, "username" => username} ->
        result = ManageUserService.get_user_by_email_and_username(email, username)
        conn
        |> put_view(json: UserPresenter)
        |> render(:present_user, user: result.user)
      _ ->
        user = ManageUserService.get_users()
        conn
        |> put_view(json: UserPresenter)
        |> render(:present_users, users: user)
      end
  end
