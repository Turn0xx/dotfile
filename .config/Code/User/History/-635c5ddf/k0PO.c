...
void insertSymbol(char* name, int type, int address) {
    int index = hash(name);
    int originalIndex = index;
    while (SS.currentScope->symbolTable[index].name[0] != '\0') {
        if (strcmp(SS.currentScope->symbolTable[index].name, name) == 0) {
    ...
    strcpy(SS.currentScope->symbolTable[index].name, name);
    SS.currentScope->symbolTable[index].type = type;
    SS.currentScope->symbolTable[index].address = address;
}
