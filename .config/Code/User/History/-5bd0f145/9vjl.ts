...
export class OrganizerTypeOrmRepository implements OrganizerRepository {
  ...
  async update(organizer: Organizer): Promise<void> {
    const organizerRepository = this.dataSource.getRepository(OrganizerSchema);
    const eventsRepository = this.dataSource.getRepository(EventSchema);
    ...
    try {
      await queryRunner.manager.withRepository(eventsRepository).save(organizer.events);
      await queryRunner.manager.withRepository(organizerRepository).save(organizer);
