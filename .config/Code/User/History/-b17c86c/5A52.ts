...
export const clientBuilder = ({
...
}: Partial<ClientJson | IndividualClientJson | CompanyClientJson> = {}) => {
  const props = {
    id,
    email,
    password,
  ...
  return {
    toIndividualClient: () => new Client(props.id, props.email, props.password, props.phoneNumber, props.createdAt),
