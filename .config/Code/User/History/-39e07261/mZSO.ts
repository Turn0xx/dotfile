...
import { TokenSchema, TokenType } from '../../../notification-services/infrastructure/tokens.schema';
...
export class InMemoryTokensRepository implements TokensRepository{
  ...
  async getDigitCodeByClientId(clientId: number): Promise<TokenSchema | undefined> {
    return await Promise.resolve(
      Array.from(this.tokens.values()).find((c) => c.client.id === clientId && c.type === TokenType.PASSWORD_RESET) as TokenSchema
    );
  }
}